...
namespace Varlık_Zimmet_Depo_Yönetimi.Varlık_Zimmet_Depo_YönetimiUI
{
    public partial class Rapor1 : Form
    {
        ...
        public Rapor1(Personel personel):this()
        {
        ...
        {
            MSSQLProvider provider = new MSSQLProvider("select * from Depo");

            SqlDataReader reader = provider.ExcuteredaerOlustur();

            if (reader.HasRows) // enaz bir satır okuduysa
            {
                ...
                while (reader.Read())
                {
                    ...
                    depo.DepoId = reader.GetInt32(0);
                    depo.DepoAdi = reader.GetString(1);
                    depo.DepoAdresi = reader.GetString(2);


                    cmbDepo.Items.Add(depo);
        ...
        {
            label3.Text = (cmbDepo.SelectedItem as Depo).DepoAdresi;

            DeponunUrunleriYukle((cmbDepo.SelectedItem as Depo).DepoId);
